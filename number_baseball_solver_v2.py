# -*- coding: utf-8 -*-
"""
Created on Sun Apr 23 00:54:50 2023
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.dark_mode_image = customtkinter.CTkImage(light_image=Image.open(image_path + "sun-fill.png"),
                                                 dark_image=Image.open(image_path + "moon-stars-fill.png"), size=(20, 20))
        
        self.restart_image = customtkinter.CTkImage(light_image=Image.open(image_path + "arrow-clockwise.png"),
                                                 dark_image=Image.open(image_path + "arrow-clockwise-white.png"), size=(30, 30))
        ...
        self.iconbitmap(image_path + "NBS.ico")
    ...
    def start(self):
        ...
        for i, candidate in enumerate(candidates, start=0):
            self.candidates_button.append(customtkinter.CTkButton(self.candidates_frame, text=candidate, width = 70, height = 35, border_width=0, corner_radius=0, text_color=("gray10", "#DCE4EE"), font=self.bold_font, command=lambda candidates=candidates, i=i: self.change_nsmb_first_run(candidates, i)))
            ...
            self.candidates_progressbar[i].set(1)
            if i == 200:
                break
    ...
    def change_nsmb_first_run(self, candidates: list, index: int):
        widgets = self.nsmb_frame.grid_slaves()
        for widget in widgets:
            widget.destroy()
            
        candidate = candidates[index]
        group = self.bb.grouping(candidates, candidate)
        group = dict(sorted(group.items(), key=lambda group: len(group[1]), reverse=True))
            
        self.nsmb_frame.configure(label_text = 'nsmb '+candidate)
        self.nsmb_button = []
        self.nsmb_progressbar = []
        for i, (key, value) in enumerate(group.items(), start=0):
            self.nsmb_button.append(customtkinter.CTkButton(self.nsmb_frame, text=key, width = 70, height = 35, border_width=0, corner_radius=0, text_color=("gray10", "#DCE4EE"), font=self.bold_font, command=lambda index=index, candidate=candidate, nsmb=key, group=group: self.change_candidates_first_run(index, candidate, nsmb, group)))
            ...
            self.nsmb_progressbar[i].set(len(value)/len(list(group.values())[0]))
            if i == 200:
                break

    def change_candidates_first_run(self, index: int, candidate: str, nsmb: str, group: dict):
        widgets = self.candidates_frame.grid_slaves()
        for widget in widgets:
            widget.destroy()
            
        self.history_label.append(customtkinter.CTkLabel(self.history_frame, width = 140, height = 35, fg_color=("blue", "dark blue"), font=self.bold_font, text=candidate + " " + nsmb))
